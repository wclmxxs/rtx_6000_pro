...
def worker_service(gpu: dict[str, object], data_root: str) -> list[str]:
    ...
    return [
        ...
        f"      - {slot}/user:/user",
        "    healthcheck:",
        "      test: ['CMD-SHELL', 'curl -fsS http://127.0.0.1:8188/system_stats >/dev/null && curl -fsS http://127.0.0.1:8188/object_info/CacheDiT_MiniMax_H3_Advanced_Optimizer | grep -q CacheDiT_MiniMax_H3_Advanced_Optimizer && curl -fsS http://127.0.0.1:8188/object_info/MiniMaxH3ScheduledSolAttentionPatch | grep -q MiniMaxH3ScheduledSolAttentionPatch']",
        "      interval: 10s",
        "      timeout: 5s",
        "      retries: 18",
        "      start_period: 90s",
...
def api_service(
    ...
    return [
        ...
        "    healthcheck:",
        "      test: ['CMD-SHELL', 'curl -fsS -H \"Authorization: Bearer $$API_KEY\" http://127.0.0.1:30010/healthz >/dev/null']",
        "      interval: 10s",
        "      timeout: 5s",
        "      retries: 12",
        "      start_period: 30s",
